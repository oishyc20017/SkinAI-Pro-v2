...
def admin_page():
    ...
    current_email = st.session_state.get("email", "").strip().lower()
    ...
    if not admin_email or current_email != admin_email:
        ...
        st.warning("You do not have permission to access the Admin Dashboard.")
        return
    ...
    c2.metric("🔬 Predictions", predictions_count)
    c3.metric("💬 Conversations", conversations_count)
    c4.metric("📨 Messages", messages_count)
    c5.metric("📅 Bookings", bookings_count)
    ...
    try:
        ...
        with open(DB_PATH, "rb") as f:
            ...
            st.download_button(
                ...
                data=f,
                file_name="skinai_cloud_backup.db",
        ...
        st.error(f"Could not prepare database download: {e}")